...
def initialize_db():
    ...
    cursor = conn.cursor()
    cursor.execute('''CREATE TABLE IF NOT EXISTS site_connections
             (id INTEGER PRIMARY KEY, site_name TEXT, site_url TEXT, active INTEGER)''')
    cursor.execute('''CREATE TABLE IF NOT EXISTS farming_tasks
             (id INTEGER PRIMARY KEY, task_name TEXT, site_id INTEGER, status TEXT,
...
def manage_site_connections():
    if request.method == 'POST':
        ...
        conn = sqlite3.connect(DATABASE_URI)
        cursor = conn.cursor()
        cursor.execute("INSERT INTO site_connections (site_name, site_url, active) VALUES (?, ?, ?)",
...
def handle_farming_tasks():
    if request.method == 'POST':
        ...
        conn = sqlite3.connect(DATABASE_URI)
        cursor = conn.cursor()
        cursor.execute("INSERT INTO farming_tasks (task_name, site_id, status) VALUES (?, ?, ?)",
